...
model = dict(
    ...
    detector=dict(  # 检测器的详细配置说明请查看 https://github.com/open-mmlab/mmdetection/blob/master/docs/tutorials/config.md#an-example-of-mask-r-cnn
        type='FasterRCNN',
        backbone=dict(
            type='ResNet',
            depth=50,
    ...
    reid=dict(  # 重识别模型的配置
        ...
        neck=dict(type='GlobalAveragePooling', kernel_size=(8, 4), stride=1),  # 重识别网络的模型颈部,通常是全局池化层。
        head=dict(  # 重识别网络的模型头部
    ...
    tracker=dict(  # 跟踪器配置
        ...
        reid=dict(  # 测试阶段重识别模块配置
            ...
            img_scale=(256, 128),  # 充识别模型输入的图片大小
...
# 以下配置与视频目标检测一直。 详情请参考 `config_vid.md`
