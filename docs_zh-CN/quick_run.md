...
- `CHECKPOINT_FILE`：如果已经在配置文件里使用 `pretrains` 关键字设置了预训练模型，那么检查点是可选的。
- `DEVICE`：推理设备。可选 `cpu` 或者 `cuda:0` 等。
- `BACKEND`：可视化锚框的后端。可选 `cv2` 或者 `plt`。
- `--show`：是否实时显示视频。
...
5. 在 MOT17 上测试 Tracktor，并且评估 CLEAR MOT metrics。
...
6. 使用 8 GPUs 测试 Tracktor，并且评估 CLEAR MOT metrics。
...
根据 [Linear Scaling Rule](https://arxiv.org/abs/1706.02677)，如果您使用不同的 GPU 或每个 GPU 中不同的 batch size，则需要设置与 batch size 成正比的学习率，例如 `lr=0.01` 用于 8 个 GPU \* 1 img/gpu 和 `lr=0.04` 用于 16 个 GPU \* 2 imgs/gpu。
...
在训练期间，日志文件和检查点将保存到工作目录中，该目录由配置文件中的 `work_dir` 或通过 CLI 的 `--work-dir` 参数指定。
...
如果您使用 `dist_train.sh` 来启动训练任务，您可以使用一下命令中设置端口：
...
1. 通过`--options`设置端口。更推荐使用此方式，因为它不会更改原始配置。
...
[tutorials/customize_dataset.md](tutorials/customize_dataset.md) 中提供了有关自定义数据集的详细信息。
...
[tutorials/config.md](tutorials/config.md) 提供了有关配置系统的更多详细信息。
