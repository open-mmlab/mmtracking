...
from mmtrack.core import (DetLocalVisualizer, TrackDataSample,
...
class TestTrackLocalVisualizer(TestCase):
    ...
    def _get_pred_data_sample():
        ...
        instances = InstanceData(**instances_data)
        data_sample = TrackDataSample()
        data_sample.pred_track_instances = instances
        return data_sample
    ...
    def test_add_datasample(self):
        ...
        gt_data_sample = self._get_gt_data_sample()
        pred_data_sample = self._get_pred_data_sample()
        track_local_visualizer = TrackLocalVisualizer(alpha=0.2)
        track_local_visualizer.dataset_meta = dict(
            CLASSES=['pedestrian', 'vehicle'])

        # test gt_instances
        track_local_visualizer.add_datasample('image', image, gt_data_sample,
                                              None)

        # test out_file
        track_local_visualizer.add_datasample(
            'image', image, gt_data_sample, None, out_file=out_file)
...
class TestDetLocalVisualizer(TestCase):
    ...
    def _get_pred_data_sample():
        ...
        instances = InstanceData(**instances_data)
        data_sample = TrackDataSample()
        data_sample.pred_det_instances = instances
        return data_sample
    ...
    def test_add_datasample(self):
        ...
        det_local_visualizer.add_datasample(
            'image',
            image,
            gt_sample=gt_data_sample,
            pred_sample=pred_data_sample,
            out_file=out_file)
        self._assert_image_and_shape(out_file, (h, 2 * w, 3))
