...
# 8 images, 2 instances, 13 objects
...
def test_dataset_evaluation():
    ...
    def _create_gt_results(dataset):
        ...
        for img_info in dataset.data_infos:
            ...
            bbox_result = bbox2result(bboxes, ann['labels'], len(classes))
    ...
    dataset = CocoVideoDataset(
        ann_file=COCO_VIDEO_ANN_FILE, classes=('car', 'person'), pipeline=[])
    ...
    assert 'MOTA' in eval_results
    assert 'IDF1' in eval_results
    assert 'IDSw.' in eval_results
