...
class YouTubeVISMetric(BaseVideoMetric):
    ...
    def _format_one_video_preds(self, pred_dicts: Tuple[dict]) -> List:
        ...
        """
        preds = self.results2dict(pred_dicts)
    ...
    def results2dict(self, results: Tuple[dict]) -> dict:
        """Convert predictions to the specified format.

        Combine the original prediction results according to
        different keys.

        Args:
            results (Tuple[dict]): Testing results of the
                dataset.

        Returns:
            dict: Possible keys are "bboxes", "masks", "scores", "labels"
            and "instance_id".
        """
        result_dict = defaultdict(list)

        keys = ['bboxes', 'masks', 'scores', 'labels', 'instance_id']
        for pred in results:
            for key in keys:
                result_dict[key].append(pred[key])
        return result_dict
