...
from .base_anchor_head import BaseAnchorHead


@SOT_HEADS.register_module()
class MultiDepthwiseRPN(BaseAnchorHead):
        ...
        for i in range(len(in_channels)):
            self.cls_heads.append(
                CorrelationHead(in_channels[i], in_channels[i],
                                2 * self.num_anchor, kernel_size, norm_cfg))
            self.reg_heads.append(
                CorrelationHead(in_channels[i], in_channels[i],
                                4 * self.num_anchor, kernel_size, norm_cfg))
    ...
    def get_bbox(self, cls_score, bbox_pred, prev_bbox, scale_factor):
        ...
        anchor = self.anchors.to(cls_score.device)
        bbox_pred = bbox_pred.permute(1, 2, 3, 0).contiguous().view(4, -1)
        bbox_pred = bbox_pred.permute(1, 0)

        # TODO: remove this line
        bbox_pred = bbox_pred.clamp(-10., 10.)

        bbox_pred[:, 0] = bbox_pred[:, 0] * anchor[:, 2] + anchor[:, 0]
        bbox_pred[:, 1] = bbox_pred[:, 1] * anchor[:, 3] + anchor[:, 1]
        bbox_pred[:, 2] = torch.exp(bbox_pred[:, 2]) * anchor[:, 2]
        bbox_pred[:, 3] = torch.exp(bbox_pred[:, 3]) * anchor[:, 3]
        ...
        # window penalty
        penalty_score *= (1 - self.test_cfg.window_influence)
        penalty_score += self.window.to(
            cls_score.device) * self.test_cfg.window_influence
