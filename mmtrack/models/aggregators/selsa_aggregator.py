...
class SelsaAggregator(nn.Module):
    ...
    def forward(self, x, ref_x):
        ...
        Args:
            x (Tensor): of shape [N, C]. N is the number of target frame
                proposals.
            ref_x (Tensor): of shape [M, C]. M is the number of ref frame
                proposals.

        Returns:
            Tensor: The aggregated features of proposals with shape [N, C].
        """
        roi_n = x.shape[0]
