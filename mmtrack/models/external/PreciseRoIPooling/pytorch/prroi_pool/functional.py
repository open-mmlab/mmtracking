...
class PrRoIPool2DFunction(ag.Function):
    ...
    def forward(ctx, features, rois, pooled_height, pooled_width,
        ...
        assert 'FloatTensor' in features.type() \
            and 'FloatTensor' in rois.type(), \
            'Precise RoI Pooling only takes float input, ' \
            'got {} for features and {} for rois.'.format(
        ...
        if features.is_cuda:
            ...
            raise NotImplementedError(
                'Precise RoI Pooling only supports GPU (cuda) implementations.'
            )
