...
class SeqTrainCollect(Collect):
    ...
    def __call__(self, results):
        ...
        assert len(outs) == len(self.ref_prefix) + 1
        data = {}
        data.update(outs[0])
        for i in range(1, len(self.ref_prefix) + 1):
            for k, v in outs[i].items():
                data[f'{self.ref_prefix[i-1]}_{k}'] = v
